                                                ...
                                                <?php
                                                    ...
                                                    while ($row = mysqli_fetch_assoc($stmt)) {
                                                        echo "<tr>
                                                            ...
                                                            <td class='text-danger'>" . htmlspecialchars($row['status']) . 
                                                                ( $row['status'] == 0 ? "disapproved" : "approved" ) . 
                                                            "</td>
                                                            <td>" . htmlspecialchars($row['title']) . "</td>
                                                            <td>" . htmlspecialchars(date("Fj Y | h:i:a", strtotime($row['created_at']))) . "</td>
                                                            <td>
                                                                <a href='" . BASE_ADMIN . "/comments/approved-comments.php?apr_Id=" . htmlspecialchars($row['id']) . "' class='btn btn-outline-success m-1' data-bs-toggle='tooltip' data-bs-title='disapproved comment'><i class='bx bx-redo'></i></a>
                                                                &nbsp;
                                                                <a href='" . BASE_ADMIN . "/comments/disapproved-comment.php?del_id=" . htmlspecialchars($row['id']) . "' class='btn btn-outline-danger m-1' data-bs-toggle='tooltip' data-bs-title='permanent delete comment'><i class='bx bx-trash-alt'></i></a>
                                                            </td>
                                                        </tr>";
                                                    }

                                                    if (mysqli_num_rows($stmt) == 0) {
                                                        echo "<p class='found' style='font-size: 16px'>No disapproved comments found.</p>";
                                                    }
